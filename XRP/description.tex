...
% # To-do list
% - Put in references. Search for all occurrences of CITE
% - Search for all occurrences of HOGG
% - Search for all occurrences of BEDELL
% - Search for all occurrences of ...
% - get management experience into biosketch
% - put some ``and others'' into the bibtex; possibly fix bst file?

\documentclass[12pt, letterpaper]{article}
\usepackage{fancyhdr, graphicx, natbib}
...
exoplanetary systems rest crucially on the extreme precision radial-velocity (\EPRV)
technique. Through \EPRV, we can detect planets in the medium-to-long 
...
detail using the Rossiter-McLaughlin effect. With the next generation of \EPRV\ 
...
(\acronym{RMS} around the mean) smaller than $\approx 100\,\cmps$ 
($1\,\mps$).\footnote{At least not in the literature! We have heard rumors and 
...
pipeline for \EPRV\ data analysis \citep[\wobble][]{Bedell2019}.
...
(two orders of magnitude more informative) (HOGG what does this mean?) and thereby transform the science
...
HOGG: rephrase the above to explicitly tie ``nuisance parameters'' to the situation 
where the spectrum is intrinsically variable?
...
HOGG: THIS IS ALL VERY VAPORWARE-SOUNDING. CAN WE ADD A 
``CASE STUDY'' OF WHAT PURSUING ONE OF THESE APPROACHES 
MIGHT LOOK LIKE? e.g. how would we simulate these things? 
what's the metric of success or failure in terms
of whether these approaches are the right ones to pursue? 
what useful things for the EPRV community would we produce - probably 
useful metrics for what kinds of stars to observe and what kind of observing 
strategy to use, right? would we use any real data to test our conclusions? 
...
Our approach on star spots, building on prior work (CITE GULLY) will
...
anomalies (HOGG CITE), temperature anomalies, or mixtures of the two.
...
use HARPS-N solar spectra! (or, failing data access, HARPS solar spectra after their 2-yr 
proprietary period is up)
